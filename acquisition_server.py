...
try:
    if sys.platform.startswith('win32'):
        libEDK = cdll.LoadLibrary("/lib/edk.dll")
    elif sys.platform.startswith('linux'):
        srcDir = os.getcwd()
	if platform.machine().startswith('arm'):
            libPath = srcDir + "/../../bin/armhf/libedk.so"
	else:
            libPath = srcDir + "/lib/libedk.so.3.3.3"
        libEDK = CDLL(libPath)
    else:
...
ALPHA_AF3 = c_double(0)
BETA_AF3  = c_double(0)
ALPHA_AF4 = c_double(0)
BETA_AF4  = c_double(0)
...
channelList = array('I',[3, 7, 9, 12, 16])   # IED_AF3, IED_T7, IED_Pz, IED_T8, IED_AF4
...
print "ALPHA_AF3, BETA_AF3, ALPHA_AF4, BETA_AF4 \n"
...
#                THETA     = THETA + thetaValue.value
            ...
            result = libEDK.IEE_GetAverageBandPowers(userID,3, theta, alpha, low_beta, high_beta, gamma)
            ALPHA_AF3 = alphaValue.value
            BETA_AF3 = low_betaValue.value + high_betaValue.value
            
            result = libEDK.IEE_GetAverageBandPowers(userID,16, theta, alpha, low_beta, high_beta, gamma)
            ALPHA_AF4 = alphaValue.value
            BETA_AF4 = low_betaValue.value + high_betaValue.value
...
#            THETA     = THETA/len(channelList)
            ...
            if result == 0:    #EDK_OK
                print "%.6f, %.6f, %.6f, %.6f \n" % (ALPHA_AF3, BETA_AF3, ALPHA_AF4, BETA_AF4)
                SOCKET.sendto("%.6f %.6f %.6f %.6f %s" % (ALPHA_AF3, BETA_AF3, ALPHA_AF4, BETA_AF4, datetime.datetime.now().strftime("%H:%M:%S.%f")), ('<broadcast>', BROADCAST_PORT))
